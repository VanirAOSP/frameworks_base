...
public class FlashNotifications {
    ...
    int mFlashTimeout = 50;
    boolean mScreenIsOn = true;
    ...
    public FlashNotifications(Context context) {
        ...
        final boolean isFlashSupported = mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA_FLASH);
        final boolean isCameraPresent = mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA);
        mAble = (isFlashSupported && isCameraPresent);

        if (mAble) registerListeners();
    }
