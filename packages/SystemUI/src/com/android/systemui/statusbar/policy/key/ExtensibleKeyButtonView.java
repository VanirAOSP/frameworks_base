...
public class ExtensibleKeyButtonView extends KeyButtonView {
    ...
    public void setActions(String clickAction, String longPress) {
        if (clickAction != null) {
            if (clickAction.equals(VanirAwesome.ACTION_HOME)) {
                setCode(KeyEvent.KEYCODE_HOME);
                setId(R.id.home);
            } else if (clickAction.equals(VanirAwesome.ACTION_BACK)) {
                setCode(KeyEvent.KEYCODE_BACK);
                setId(R.id.back);
            } else if (clickAction.equals(VanirAwesome.ACTION_MENU)) {
                setCode(KeyEvent.KEYCODE_MENU);
                setId(R.id.menu);
            } else if (clickAction.equals(VanirAwesome.ACTION_POWER)) {
                setCode(KeyEvent.KEYCODE_POWER);
            } else if (clickAction.equals(VanirAwesome.ACTION_SEARCH)) {
    ...
    protected void setLongPress() {
        ...
        if (mLongpress != null) {
			if (!mLongpress.equals(VanirAwesome.ACTION_HOME)) {
				setSupportsLongPress(false);
		    } else if ((!mLongpress.equals(VanirAwesome.ACTION_NULL)) || (getCode() != 0)) {
    ...
    protected OnLongClickListener mLongPressListener = new OnLongClickListener() {
        ...
        public boolean onLongClick(View v) {
            return VanirAwesome.getInstance(mContext).launchAction(mLongpress);
        }
    };
}
