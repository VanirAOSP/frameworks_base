...
public class ActiveDisplayView extends FrameLayout {
    ...
    private void setActiveNotification(final StatusBarNotification sbn, final boolean updateOthers) {
        try {
            ...
            float larger = (float)Math.max(centerDrawable.getWidth(),centerDrawable.getHeight());
            if (larger > 0) {
                centerDrawable.setScaleX(1.0f * mIconSize / larger);
                centerDrawable.setScaleY(1.0f * mIconSize / larger);
            }
